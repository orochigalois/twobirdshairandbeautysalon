...
$outline_custom_hover_text = $add_icon = $i_align = $i_type = $i_icon_entypo = $i_icon_fontawesome = $i_icon_linecons = $i_icon_pixelicons = $i_icon_typicons = $css = $css_animation = $row_use_extended_content = '';
...
/*ozy custom*/
if( $row_use_extended_content === 'on' && $content) {
	wp_enqueue_style('extended-box');
	wp_enqueue_script('extended-box');
	$random_extended_box_id = 'ozy-ecb' . rand( 0, 100000 );
	echo '<div style="display:none" id="'. esc_attr( $random_extended_box_id ) .'">'. wpb_js_remove_wpautop( $content ) .'</div>';
	$attributes[] = 'data-extended-content="' . esc_attr( $random_extended_box_id ) . '"';
}
