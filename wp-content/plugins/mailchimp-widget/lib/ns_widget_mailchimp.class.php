...
class NS_Widget_MailChimp extends WP_Widget {
	...
	public function __construct () {
		...
		$this->default_signup_text = __('Subscribe');
		...
		parent::__construct('ns_widget_mailchimp', __('NEWSLETTER', 'mailchimp-widget'), $widget_options);
	...
	public function form ($instance) {
		...
		if (false == $mcapi) {
			...
			?>
					...
					<p>
						<label for="<?php echo $this->get_field_id('title2'); ?>"><?php echo  __('Title #2:', 'mailchimp-widget'); ?></label>
	...
	public function widget ($args, $instance) {
		...
		/*if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
			...
			//echo $before_widget . $before_title . $instance['title'] . ($instance['title2'] ? '<p>'. $instance['title2'] .'</p>':'') . $after_title;
			echo $before_widget . '<p class="mailchimp-widget-p">' . $before_title . $instance['title'] . $after_title . ($instance['title2'] ? '<p class="mailchimp-widget-p">'. $instance['title2'] .'</p>':'');
			
			if ($this->successful_signup) {
				echo $this->signup_success_message;
			} else {
				?>	
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" class="<?php echo $this->id_base . '_form'; ?>" id="<?php echo $this->id_base . '_form-' . $this->number; ?>" method="post">
					...
					?>
						...
						<input id="<?php echo $this->id_base; ?>-email-<?php echo $this->number; ?>" type="email" name="<?php echo $this->id_base; ?>_email" placeholder="<?php echo __('Email Address', 'mailchimp-widget'); ?>" />
						<input class="button" id="mc_signup_submit" type="submit" name="<?php echo __($instance['signup_text'], 'mailchimp-widget'); ?>" value="<?php echo __($instance['signup_text'], 'mailchimp-widget'); ?>" />
					</form>
