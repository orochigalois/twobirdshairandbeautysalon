 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
	    ...
	    public function add_admin() {
			...
			add_theme_page("Import Demo Data", "Import Demo Data", 'switch_themes', 'ozy_demo_installer', array($this, 'demo_installer'));
    	...
    	public function intro_html() {
			$memory_limit = salon_return_bytes(ini_get('memory_limit'));
			$max_execution_time = ini_get('max_execution_time');
			$post_max_size = ini_get('post_max_size');
			$upload_max_filesize = ini_get('upload_max_filesize');

			echo '<h3>Recommended PHP Configuration Limits</h3>';
			echo '<p class="generic-info-box white-box">Many reported issues you may run into such as; white screen, demo content fails when importing and other similar issues are all related to low PHP configuration limits. The solution is to increase the PHP limits. You can do this on your own, or contact contact your web host and ask them to increase those limits to a minimum as follows:</p>';
			echo '<table width="400px" class="server_resources_table">';
			echo '	<tbody>';
			echo '	<tr><th>&nbsp;</th><th>Available</th><th>Recommended</th></tr>';
			echo '	<tr><td>Memory Limit</td><td class='. salon_check_limit_size($memory_limit, 67108864) . '>'. ($memory_limit / 1024 / 1024) .'M</td><td>64M</td></tr>';
			echo '	<tr><td>Max Execution Time</td><td class='. salon_check_limit_size($max_execution_time, 180) . '>'. $max_execution_time .'s</td><td>180s</td></tr>';
			echo '	<tr><td>Post Max Size</td><td class='. salon_check_limit_size($post_max_size, 32) . '>'. $post_max_size .'</td><td>32M</td></tr>';
			echo '	<tr><td>Upload Max Size</td><td class='. salon_check_limit_size($upload_max_filesize, 32) . '>'. $upload_max_filesize .'</td><td>32M</td></tr>';					
			echo '	</tbody>';
			echo '</table>';
			echo '<p>We strongly recommend to check and follow available limits as shown above before processing any further.</p>';
			?>

			<div class="generic-info-box blue-box">
			 ...
			 <div class="generic-info-box blue-box"><p class="tie_message_hint">Before you begin, make sure all the required plugins are activated.</p></div>

			 <?php

			 if( !empty($this->imported_demos) ) { ?>

			  	<div class="generic-info-box green-box">
			  		<p><?php esc_attr_e('Demo already imported', 'radium'); ?></p>
			  	</div><?php
			   	//return;
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_attr__('Import Again', 'radium');

			} else {

				$button_text = esc_attr__('Import Demo Data', 'radium');
	       ...
	       <div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
			...
			<?php
			echo '<h3>Revolution Sliders</h3>';
			echo '<p>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="featured_products" checked>Featured Products</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="Featured_Services" checked>Featured Services</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="featured-news" checked>Featured News</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="hotspot_banner" checked>Hotspot Banner</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="instagram-gallery" checked>Instagram Gallery</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="Media-Center" checked>Media Center</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="model-carousel" checked>Model Carousel</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="our-team-carousel5" checked>Our Team Carousel</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="Service-Carousel" checked>Service Carousel</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="Spa_wellness" checked>Spa Wellness</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="typewriter-effect" checked>Front Page</label><br>';
			echo '</p>';
			
			echo '<div id="ozy_import_revo_info"></div>';
			
			echo '<div class="generic-info-box orange-box"><p class="tie_message_hint">Any existing slider with same alias / name might be overridden. Please select fewer items for each import if you are having difficulties to import sliders.</p></div>';
			
			echo '<p><input type="button" class="button button-primary button-large" name="ozy_import_revo_slider" id="ozy_import_revo_slider" value="Import Selected Sliders"></p>';
	    ...
	    public function set_demo_data( $file ) {
	        ...
	        if ( !class_exists( 'WP_Import' ) ) {
	            ...
	            //$class_wp_import = get_template_directory() . '/samples/importer/wordpress-importer.php';
	        ...
	        if($importer_error){
	            ...
	            if(!is_file( $file )){
					...
					$GLOBALS['wp_import'] = $wp_import; //please see framework/includes/wpalchemy/metabox.php line 534
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_attr__( 'Theme options Import file could not be found. Please try again.', 'radium' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_attr__( 'Widget Import file could not be found. Please try again.', 'radium' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_attr__( 'Sidebar does not exist in theme (using Inactive)', 'radium' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_attr__( 'Site does not support widget', 'radium' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_attr__( 'Widget already exists', 'radium' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_attr__( 'Imported', 'ozy-salon-essentials' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_attr__( 'Imported to Inactive', 'ozy-salon-essentials' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_attr__( 'No Title', 'ozy-salon-essentials' ); // show "No Title" if widget instance is untitled
